...
describe("useFormalNative()", () => {
  it("should extend useFormal()", () => {
    ...
    const formalNative = renderHook(() => useFormalNative(initialValues, { onSubmit: (values) => values }));
  ...
  describe(".getFormProps()", () => {
    it("should throw an error", () => {
      const { result } = renderHook(() => useFormalNative(initialValues, { onSubmit: (values) => values }));
  ...
  describe(".getFieldProps()", () => {
    describe(".onChangeText()", () => {
      it("should call change() with the current field value", () => {
        ...
        expect(result.current.getFieldProps("firstName").onChangeText).toBeTruthy();
      });
    });
  });
