...
export default function useFormal<Schema>(
  ...
  const isDirty = useMemo(() => !isEqual(lastValues, values), [lastValues, values]);

  const isValid = useMemo(() => !isDirty || objectIsEmpty(errors), [errors, isDirty]);

  const change = useCallback((field: keyof Schema, value: any): void => {
  ...
  const validate = useCallback(() => {
    if (!schema) throw new Error("You cannot call validate if you have not provided any schema.");
  ...
  const getResetButtonProps = useCallback(
    () => ({
      disabled: (!isDirty && objectIsEmpty(errors)) || isValidating || isSubmitting,
    }),
  ...
  const getSubmitButtonProps = useCallback(
    () => ({
      disabled: (!isDirty && !isSubmitted && objectIsEmpty(errors)) || isValidating || isSubmitting,
    }),
